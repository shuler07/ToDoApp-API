...
from auth import auth, config, decode_token, is_token_valid, get_uid_from_token
...
@app.post('/refresh', summary='Refresh token', tags=['Authentication'], dependencies=[Depends(auth.refresh_token_required)])
def refresh_token(response: Response, refresh_token: str = Cookie(None, alias=config.JWT_REFRESH_COOKIE_NAME)):
    print('Refreshing token', refresh_token)
    try:
        payload = decode_token(refresh_token)
        print('payload:', payload)
        new_access_token = auth.create_access_token(payload.get('sub'))
        new_refresh_token = auth.create_refresh_token(payload.get('sub'))
...
def register(creds: UserCredsSchema, response: Response, session: sessionDependency):
...
def login(creds: UserCredsSchema, response: Response, session: sessionDependency):
    query = select(UserModel).where(UserModel.email == creds.email)
    result = session.execute(query)
    user = result.scalar_one_or_none()
    ...
    access_token = auth.create_access_token(uid=str(user.uid))
    refresh_token = auth.create_refresh_token(uid=str(user.uid))
    response.set_cookie(config.JWT_REFRESH_COOKIE_NAME, refresh_token)
    return {'isLoggedIn': True, 'access_token': access_token, 'uid': user.uid}
        
@app.post('/signout', summary='Sign out', tags=['Authentication'])
def sign_out(userAuth: UserAuthSchema, response: Response, refresh_token: str = Cookie(None, alias=config.JWT_REFRESH_COOKIE_NAME)):
    access_token = userAuth.access_token
...
@app.post('/create_note', summary='Create note', tags=['Notes'])
def create_note(createNote: CreateNoteSchema, session: sessionDependency):
    access_token = createNote.access_token
    if access_token is None:
        raise HTTPException(401, 'User not logged in')
    
    uid = get_uid_from_token(access_token)
    if uid is None:
        raise HTTPException(401, 'Invalid token')
    
    new_note = NotesModel(
        uid=int(uid),
        note=createNote.note_text,
...
def get_notes(userAuth: UserAuthSchema, session: sessionDependency):
    access_token = userAuth.access_token
    if access_token is None:
        raise HTTPException(401, 'User not logged in')
